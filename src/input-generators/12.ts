...
// https://github.com/ClemDelp/heightmap-generator/blob/master/heightmap.js
function generateNoise(
    ...
    return flatten(
        interpolate(noiseArr, squareSize),
        level,
        revert,
        valueDivisor
    );
...
function interpolate(points: any, squareSize: any) {
    var noiseArr = new Array();
    var x = 0;
    var y = 0;
    ...
    for (let i = 0; i < size; i++) {
        ...
        for (let j = 0; j < size; j++) {
            ...
            var int_x2 =
                points[x][y + 1] * (1 - mu_2) + points[x + 1][y + 1] * mu_2;
...
function flatten(points: any, level: any, revert: any, valueDivisor: any) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        for (let j = 0; j < points[i].length; j++) {
            for (let k = 0; k <= level; k++) {
                var val = step * k; // Math.round(step * k * 10) / 10;
                if (val === maxVal) {
...
const noise = generateNoise(10, 26, true, 1);
