...
const SOLUTION = chain(process.argv[2]?.split("-")).then(([dayStr, partStr]) => [Number(dayStr), Number(partStr)]).value;
...
if (WITH_PERF) {
    new Benchmark.Suite()
        ...
        const logStr = stringBuilder("[")
            ...
            .append(expected === undefined || result === expected
            ? `\n       got ${chalk.bold.green(result)} `
            : `\n       expected ${chalk.bold.green(expected)} got ${chalk.bold.red(result)} `)
            .append(`in ~${periodMs.toFixed(4)} ms (${periodUs.toFixed(4)} µs)`).value;
        console.log(logStr);
    })
        .run();
}
    ...
    const logStr = stringBuilder("[")
        ...
        .append(expected === undefined || result === expected
        ? `got ${chalk.bold.green(result)} `
        : `expected ${chalk.bold.green(expected)} got ${chalk.bold.red(result)} `).value;
    console.log(logStr);
}
